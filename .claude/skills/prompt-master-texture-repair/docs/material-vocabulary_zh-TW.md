...
供步驟 2（物理材質重生策略）參考的詞彙庫。挑選符合已解析圖像類別的條目，以繁體中文編織進 `[極致物理肌理]` 模組。這些是「調色盤」而非「檢查清單」：只選該主體實際具備的質感，不要把每個詞都塞進去。

## 目錄
1. 人物（People）
2. 產品（Products）
3. 場景與環境（Scenes）
4. 背景去噪詞庫（Background De-noise Bank）
5. 攝影設備與調性詞庫（Photography & Tone Bank）

---

## 1. 人物（People）

最大的失敗模式是塑料／過度磨皮的皮膚。務必還原微觀細節與皮膚的透光感。

- **皮膚 Skin** —— 自然彈性皮膚、真實細微毛孔、次表面散射（subsurface scattering）的健康透光感、自然膚色微不均與淡淡潮紅、細絨毛、輕微油光與乾濕分區、保留痣與雀斑等真實特徵；**未經修飾的原始攝影質感**，絕無塑料感、蠟感或過度磨皮。
...
背景子句必須「強力」—— 否則 Img2Img 會重新參考髒污原圖而去噪不徹底。將以下數項組合進 `[環境與背景]`：
