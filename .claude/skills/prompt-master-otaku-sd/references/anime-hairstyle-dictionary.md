# アニメ女性キャラ髪型辞典 (Anime Female Hairstyle Dictionary for SD Prompts)

キャラクターの髪型を正確にプロンプトで再現するための実測済みタグリファレンスです。
基礎モデル (Nova Anime XL) + LoRA (748cmStyle) で検証済み。

> **使い方**: 「単体Tag」はそのまま使用。「組合せTag」は `+` で示した複数タグを併記すること。
> **⚠ 注意**: 瀏海(前髪)の精密なコントロールはプロンプトだけでは限界がある。特に「中分」は専用LoRAが必要。
...
| 16 | 中分 | `center part bangs` | 要LoRA⚠ | LoRA「Parted bangs」が必要。プロンプトだけでは安定しない |
...
> 単色タグ（`blonde hair` 等）は `sd-tags.md` 参照。ここでは複雑な色彩表現をカバー。
...
- **中分 (Center Part)**: プロンプトだけでは不安定 → LoRA「Parted bangs」が必要
- **額出し (Forehead)**: `forehead` 単独では不十分 → `alice band` や `hair slicked back` を併用
- **Bowl Cut**: 完璧な再現は難しく、毛先が残る傾向
- **Drill Hair**: 単体で1本になりがち → `twin drills` で両側を明示
- **Very Long Hair vs Long Hair**: 差が出にくいモデルあり → `hair past waist` を追加で強調
- **Wet Hair**: 全体が塗れすぎる → シーン(rain, shower)と組み合わせると自然
- **Asymmetrical Hair**: 無視されるモデルあり → `short on one side` を補強タグとして追加
