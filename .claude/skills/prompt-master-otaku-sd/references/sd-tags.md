...
- **ダイナミックアングル強化 (Dynamic Angle / 呆板回避)**: worm's eye view (蟲視角・極端な仰角／`from below`+`dynamic angle`と併用), bird's eye view (鳥視角・真俯瞰), looking up at viewer (俯瞰で見上げられる視線), looking down at viewer, foreshortening (パース誇張), wide angle lens, three-quarters view (斜め正面・側面回避時の代替)
