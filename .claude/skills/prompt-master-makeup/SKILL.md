...
- **Architectural intent (as illusion, not surgery)** — Every technique called out MUST serve a stated structural goal (e.g., "highlight to visually elevate the cheekbone"), not just "contour". But that goal is achieved with trompe-l'œil light, shadow, and pigment ONLY — it must NEVER actually change the subject's real bone structure or feature proportions. Structural intent is always subordinate to the **Facial identity lock** below.
...
- **Facial identity lock by default** — When working from a source portrait, the compiled prompt MUST embed an explicit *identity-lock descriptor* naming the subject's real, fixed facial markers (forehead height, eye spacing / interocular distance, eye shape & lid type, brow shape & height, nose length·width·tip, mouth width & lip proportion, philtrum, cheekbone position, face shape, jaw & chin) plus a clause that the makeup adds ONLY color, texture, and soft dimensional shading — it must NOT slim, narrow, enlarge, shorten, or re-place any feature. Makeup enhances features *in place*; identity stays 100% unchanged. See `references/makeup-knowledge-base.md` §8 for the checklist and a reusable template. Relaxed only with no reference image, or when the user explicitly asks to alter a feature.
...
> **Reshape = optical illusion only.** The "reshape / alter proportion" wording in this table (Bone Structure & Contour, Lip Sculpting) describes *visual* effects from light and pigment. On a source portrait it NEVER overrides the **Facial identity lock** — real bone structure, feature placement, and proportions stay exactly as photographed.
...
- Inventory **identity-lock markers** (source-portrait only) — the fixed proportions that must NOT drift: forehead height, eye spacing / interocular distance, eye shape & lid type, brow shape & height, nose length·width·tip, mouth width & lip proportion, philtrum, cheekbone position, face shape, jaw & chin. These become the identity-lock descriptor emitted in Step 3. See knowledge base §8.
...
2. **Facial Identity Lock** (source-portrait only) — immediately after the subject, embed the identity-lock descriptor from Step 1: name the real fixed markers (esp. forehead height and eye spacing, the two most often distorted) and add the clause that makeup adds ONLY color/texture/soft shading and must NOT reshape, slim, narrow, or re-place any feature. Pull phrasing from knowledge base §8.
3. **Detailed Makeup Application** — skin/complexion → contour/highlight → brows & eyes → lips. Use exact professional terminology from the knowledge base; every term must trace back to a stated structural goal, and every reshaping-flavored term (contour, liner wing, lip line) MUST be phrased as in-place optical illusion paired with a preservation assertion (e.g., "soft cheek-hollow shadow for dimension only, face width unchanged").
4. **Composition & Lighting** — When a source portrait exists (the default case), LOCK the original composition: same shot distance & framing, same pose & gaze, same background; keep the original lighting and adapt it only minimally so the hero technique stays readable (e.g., a fractionally elevated key so a cut-crease lid still reads) without disturbing the frame or background. Only when there is NO source image — or the user explicitly requests a reframe — choose framing and lighting freely to showcase the work (e.g., a bold matte lip benefits from soft frontal light; a cut crease needs a slightly elevated key to throw the lid shadow).
...
**English Prompt:**
```text
[Single cohesive English paragraph: Subject & vibe → Facial identity lock (source-portrait: fixed markers + "makeup adds color/texture/shading only, no reshaping") → Skin/complexion → Contour/highlight (as in-place illusion) → Brows/eyes → Lips → Composition lock (default: same shot distance, pose, gaze & background) & lighting. Use exact professional terminology from the knowledge base. No tag lists, no comma-dumps.]
```
...
- **No reshaping the real face** — on a source portrait, never use verbs that alter true anatomy (slim, narrow, shorten, enlarge, widen, lift the actual feature, restructure the eye, close the interocular gap, overline to resize the lips, reshape the nose/jaw/chin). Contour, liner, and lip technique may add *optical* dimension ONLY, and each must be paired with a preservation assertion. A compiled source-portrait prompt is invalid if it omits the Facial identity lock. Relaxed only when the user explicitly requests a feature change.
